...
std::vector<Card> DeckObjectAdapter::getAllCardsFlat() const {
    ...
    for (int deckIdx = 0; deckIdx < deck.getDeckCount(); deckIdx++) {
        ...
        int currentIndex = deck.getRemainingAmountInDeck(deckIdx); // Нужно оставшееся количество карт т.к. в векторах колод не удаляются карты при раздачи, 
...
std::vector<std::vector<Card>> DeckObjectAdapter::getCardsSortedBySuit() const {
